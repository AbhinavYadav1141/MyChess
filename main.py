...
class Manager(ScreenManager):
    ...
    def new_game(self):
        if not self.games:
            self.btn = TGLButton(text=f"Home", on_press=lambda bt: self.change_scr(bt, "mainscreen"))
        ...
        self.btn = TGLButton(text=f"{len(self.games)}", on_press=lambda bt: self.change_scr(bt, name))
        self.btn.group = "screens"
        self.l2.add_widget(self.btn)
